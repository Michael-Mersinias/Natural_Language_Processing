...
# Decoder implemented in a similar manner as the Encoder
class RNNDecoder(nn.Module):
    def __init__(self, emb_dim: int, hidden_size: int, output_size: int):
        super(RNNDecoder, self).__init__()
        self.emb_dim = emb_dim
        self.hidden_size = hidden_size
        self.output_size = output_size

        self.rnn = nn.LSTM(self.emb_dim, self.hidden_size, num_layers=1, batch_first=True, dropout=0)
    ...
    def forward(self, x, hidden):
        ...
        output = torch.nn.functional.log_softmax(self.linear(h_n[0][0]), dim=1)
        return output, (h_n[0][0], h_n[1][0])


# Attention decoder implementation based on:
# https://drive.google.com/file/d/1kE1w50f8SqXlr2rpLSF2ojsW5hjLGean/view
...
class RNNAttentionDecoder(nn.Module):
    ...
    def forward(self, x, hidden, enc_outputs):
        ...
        enc_outputs = enc_outputs.view(-1, self.hidden_size)

        e_ij = torch.tanh(torch.mm(self.attention(h_n[0][0]), enc_outputs.t()))
        a_ij = torch.nn.functional.softmax(e_ij, dim=1)
        c_i = torch.mm(a_ij, enc_outputs)
        concat = torch.cat([c_i, h_n[0][0]], dim=1)

        output = torch.nn.functional.log_softmax(self.linear(concat), dim=1)

        return output, (h_n[0][0], h_n[1][0])


class Seq2SeqSemanticParser(nn.Module):
    def __init__(self, input_indexer, output_indexer, emb_dim, hidden_size, output_max_len, embedding_dropout=0.2, bidirect=True):
    ...
    def decode(self, test_data: List[Example]) -> List[List[Derivation]]:
        ...
        all_test_input_data = make_padded_input_tensor(test_data, self.input_indexer, test_input_max_len, reverse_input=False)

        # Create a list of data that corresponds to the indexed input
        batch_x = []
        for j in range(len(all_test_input_data)):
            batch_x.append(all_test_input_data[j])

        batch_x = np.array(batch_x)

        # Calculate non-zero elements which correspond to the respective lengths
        input_lengths = np.sum(batch_x != 0, axis=1)

        final_list = []
        for k in range(len(all_test_input_data)):

            # Transform to tensors and encode input
            x_tensor = torch.tensor([batch_x[k]])
            x_len_tensor = torch.tensor([input_lengths[k]])
            enc_output, enc_context_mask, enc_final_states = self.encode_input(x_tensor, x_len_tensor)

            out = []
            out_idx = self.output_indexer.index_of(SOS_SYMBOL)
            h_n = enc_final_states
            # Decoder loop
            for i in range(self.output_max_len):
                emb = self.output_emb.forward(torch.tensor(out_idx).unsqueeze(0))
                # output, h_n = self.decoder.forward(emb.unsqueeze(0), h_n)
                output, h_n = self.decoder.forward(emb.unsqueeze(0), h_n, enc_output)
                h_n = (h_n[0].unsqueeze(0), h_n[1].unsqueeze(0))
                out_idx = torch.max(output, dim=1)[1].item()

                # Terminate the loop once EOS is reached
                if out_idx == self.output_indexer.index_of(EOS_SYMBOL):
                    break

                # Append to output list
                out.append(self.output_indexer.get_object(out_idx))

            final_list.append([Derivation(test_data[k], 1.0, out)])
...
def train_model_encdec(train_data: List[Example], dev_data: List[Example], input_indexer, output_indexer, args) -> Seq2SeqSemanticParser:
    ...
    seq2seq = Seq2SeqSemanticParser(input_indexer, output_indexer, emb_dim, hidden_size, output_max_len, embedding_dropout=0.2, bidirect=True)
    ...
    optimizer = torch.optim.Adam(
        [
            {'params': seq2seq.input_emb.parameters()},
            {'params': seq2seq.encoder.parameters()},
            {'params': seq2seq.output_emb.parameters()},
            {'params': seq2seq.decoder.parameters()}
        ],
        lr=lr
    )

    # Generate an index for each character of the all_train_input_data list
    dataset_ex_indices = [i for i in range(0, len(all_train_input_data))]

    seq2seq.input_emb.train()
    seq2seq.encoder.train()
    seq2seq.output_emb.train()
    seq2seq.decoder.train()

    for epoch in range(num_epochs):
        print("Epoch: %d" % epoch)
        # New Epoch - Adjust learning rate accordingly and shuffle the data
        # Learning rate: 0.001 (epochs 0-14) -> 0.0001 (epochs 15-29)
        if (epoch+1) % 15 == 0:
        ...
        for i in range(0, len(dataset_ex_indices), batch_size):
            ...
            # Create lists of data that corresponds to the aforementioned batch of indices
            batch_x = []
            batch_y = []
            for j in range(len(batch_ex_indices)):
                batch_x.append(all_train_input_data[batch_ex_indices[j]])
                batch_y.append(all_train_output_data[batch_ex_indices[j]])
            ...
            # Calculate non-zero elements which correspond to the respective lengths
            input_lengths = np.sum(batch_x != 0, axis=1)
            output_lengths = np.sum(batch_y != 0, axis=1)
            ...
            for k in range(len(batch_x)):
                ...
                x_tensor = torch.tensor([batch_x[k]])
                x_len_tensor = torch.tensor([input_lengths[k]])
                enc_output, enc_context_mask, enc_final_states = seq2seq.encode_input(x_tensor, x_len_tensor)
                ...
                loss = 0.0
                inp_idx = output_indexer.index_of(SOS_SYMBOL)
                h_n = enc_final_states
                mode = "Scheduled Sampling"  # "Teacher Forcing" or "Scheduled Sampling"
                # Decoder loop
                for truth_idx in batch_y[k]:
                    emb = seq2seq.output_emb.forward(torch.tensor(inp_idx).unsqueeze(0))
                    # output, h_n = seq2seq.decoder.forward(emb.unsqueeze(0), h_n)
                    output, h_n = seq2seq.decoder.forward(emb.unsqueeze(0), h_n, enc_output)
                    h_n = (h_n[0].unsqueeze(0), h_n[1].unsqueeze(0))

                    # Calculate loss
                    loss += seq2seq.loss(output, torch.LongTensor([truth_idx]))

                    # Terminate the loop once EOS is reached
                    if truth_idx == output_indexer.index_of(EOS_SYMBOL):
                        break

                    # Pick between Teacher Forcing and Scheduled Sampling
                    # In case of Scheduled Sampling we start with p=1 (Teacher Forcing) and decay it as epochs increase
                    if mode == "Teacher Forcing":
                        p = 1
                    elif mode == "Scheduled Sampling":
                        decay = ((num_epochs - epoch) / num_epochs) - 0.2
                        p = random.uniform(decay, 1)
                    else:
                        print("Error")
                        p = 0

                    # We always feed the correct element if Teacher Forcing or we do so sometimes if Scheduled Sampling
                    threshold = 0.5
                    if p > threshold:
                        inp_idx = truth_idx
                    else:
                        inp_idx = torch.max(output, dim=1)[1].item()

                # Train the model and calculate the total loss for the corresponding epoch
                optimizer.zero_grad()
                total_loss += loss
                loss.backward()
                optimizer.step()
        ...
        # Early stopping based on an optimal observed epoch total loss value
        if total_loss < 50:
            break

    seq2seq.input_emb.eval()
    seq2seq.encoder.eval()
    seq2seq.decoder.eval()
    seq2seq.output_emb.eval()
