...
def log_likelihood(model: LanguageModel, some_text: str):
    ...
    :return: float
    """
    return (model.predict_all(some_text)[:, :-1] * utils.one_hot(some_text)).sum()
...
def sample_random(model: LanguageModel, max_length: int = 100):
    ...
    while len(sentence) < max_length and char != '.':
        char = utils.vocab[(torch.distributions.categorical.Categorical(logits=model.predict_next(char))).sample().item()]
        sentence += char
