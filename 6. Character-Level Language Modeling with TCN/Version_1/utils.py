...
if __name__ == "__main__":
    ...
    from os import path
    first = torch.ones(len(vocab))
    transition = torch.ones(len(vocab), len(vocab))
    for d in data:
        first += d[:, 0]
        transition += (d[:, None, :-1] * d[None, :, 1:]).sum(dim=2)
    first = (first / first.sum()).log()
    transition = (transition / transition.sum(dim=1, keepdim=True)).log()
    torch.save((first, transition), path.join(path.dirname(path.abspath(__file__)), 'bigram.th'))
